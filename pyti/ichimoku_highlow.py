...
def senkou_a(data):
    ...
    # shift forward
    #shift_by = np.repeat(np.nan, 5)
    #sa = np.append(shift_by, sa)
    return sa


def senkou_b(data, period=10):
    """
...
def senkou_h(data):
    """
    Senkou H (Leading span H)
    Formula:
    ( senkouA[chikuspan] + senkouB[chikuspan])
    """
    sh = max(senkou_a[5::], senkou_b[5::])
    return sh
